...
import {
  ...
  StyleSheet, ActivityIndicator, KeyboardAvoidingView, Platform, Modal, Image,
} from 'react-native';
...
function toHRDetail(log: any): HRDetail {
  return {
    id:                      log.id,
    peak_hr:                 log.peak_hr ?? null,
    avg_hr:                  log.avg_hr ?? null,
    hr_recovery_1min:        log.hr_recovery_1min ?? null,
    hr_recovery_2min:        log.hr_recovery_2min ?? null,
    zone_minutes:            log.zone_minutes ?? null,
    hr_screenshot_url:       log.hr_screenshot_url ?? null,
    hr_curve_screenshot_url: log.hr_curve_screenshot_url ?? null,
  };
}
...
function DayCard({
  ...
  const [openHR, setOpenHR] = useState<HRDetail | null>(null);
  ...
  return (
    ...
    <View style={[styles.dayCard, isToday && styles.dayCardToday, { borderLeftWidth: isToday ? 2 : 0, borderLeftColor: '#e8ff47' }]}>
      ...
      {expanded && !isRest && [...(day.sessions ?? [])].sort((a, b) => (a.time === 'PM' ? 1 : 0) - (b.time === 'PM' ? 1 : 0)).map((session, si) => {
        ...
        const hrDetail    = completedSessionKeys.get(sessionKey) ?? null;
        ...
        <View key={`${session.name}|${session.time ?? ''}`}>
          ...
          {(session.log_result !== false) && (
            sessionDone ? (
              <TouchableOpacity
                style={pd.completeBtn}
                onPress={() => { if (hrDetail) setOpenHR(hrDetail); }}
                disabled={!hrDetail}
                activeOpacity={0.75}
              >
                <Text style={pd.completeBtnTxt}>SESSION COMPLETE ✓</Text>
                {hrDetail && (
                  <Feather name="chevron-right" color={Colors.green} size={16} style={{ position: 'absolute', right: 16, top: 18 }} />
                )}
              </TouchableOpacity>
            ) : (
              <TouchableOpacity
    ...
    {openHR && <HRDetailModal detail={openHR} onClose={() => setOpenHR(null)} />}
  </>
  );
}
...
function HRDetailModal({ detail, onClose }: { detail: HRDetail; onClose: () => void }) {
...
export default function ProgramScreen() {
  ...
  const [completedSessionKeys, setCompletedSessionKeys] = useState<Map<string, HRDetail>>(new Map());
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (raw) {
        ...
        if (Date.now() - (c.timestamp ?? 0) < 4 * 60 * 60 * 1000) {
          ...
          const sKeys = new Map<string, HRDetail>();
          for (const log of c.sessionLogs ?? []) {
            ...
            if (log.session_name) {
              sKeys.set(`${log.week_number}|${log.day_name}|${log.session_name}|${log.session_time ?? ''}`, toHRDetail(log));
            }
            if (log.log_field) {
    ...
    for (const log of logsRes.data ?? []) {
      ...
      if (sName) {
        sKeys.set(`${log.week_number}|${log.day_name}|${sName}|${sTime ?? ''}`, toHRDetail(log));
      }
      if (log.log_field) {
        if (!tMap[log.week_number]) tMap[log.week_number] = new Set();
        tMap[log.week_number].add(`${log.day_name}:${log.log_field}`);
